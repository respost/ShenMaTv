...
$uid=$neirong[id];
$row=getone("select * from uboxfjl WHERE type='8'  and uid='".$uid."' and zyid='".$playid."'");
if (empty($row))
{
$row2=getone("select * from ubozb WHERE id='".$playid."'");
$room=intval($row2['room']);
$enroll=intval($row2['enroll']);
if ($enroll>=$room){
echo msglayer("房间已满！",8);
exit;
}else{
if ($apache==1)
{echo msglayerurl("连接中，请稍后...",8,$play."_".$playid.".html");}
else
{echo msglayerurl("连接中，请稍后...",8,$play."?playid=".$playid."&ly=ubosk");}
exit;  
}
}else{
if ($apache==1)
...
{
if ($apache==1)
{header('Location: '.$play.'_'.$playid.'.html');}
else
...
upalldt(uboip,$type);
if ($apache==1)
{echo msglayerurl($mt."观看剩余".$cishu."次！",8,$play."_".$playid.".html");}
else
