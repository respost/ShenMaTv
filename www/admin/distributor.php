...
upalldt(ubofx,$type);
echo msglayerurl("审核成功，返回页面",8,"distributor.php");
}
if($del=="del"){
$type="id='$delid'";
dbdel(ubofx,$type);
echo msglayerurl("删除成功，返回页面",8,"distributor.php");
}
$array = $_POST["del_id"]; 
if(!empty($array)){
$del_sun=count($array); 
...
echo"<script>alert('删除成功');location.href='distributor.php'</script>";  
}
?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<li class="layui-this"><a href="distributor.php">分销人员</a></li>
</ul>
...
<th width="10%" align="center" >分销链接</th>
<th width="8%" align="center" >下线人数</th>
<th width="10%" align="center" >分销金额</th>
<th width="8%" align="center" valign="middle" >等级</th>
<th width="10%" align="center" valign="middle" >分销级别</th>
<th width="10%" align="center" valign="middle" >分成比例</th>
...
<td align="center"><?php echo $a[grade];?> 级</td>
<td align="center"><?php echo $a[level];?></td>
<td align="center"><?php echo $a[divide];?> %</td>
<td align="center"><?php $status=$a[status];?><?php if ($status==1){echo "待审核";}elseif ($status==2){echo "开通成功";}?></td>
<td align="center"><?php echo $sctime."前";?></td>
<td align="center"><?php if ($status==1){?><a href="?action=shtg&id=<?php echo $a[id]?>" class="layui-btn layui-btn-normal layui-btn-mini"  target="msgubotj">通过</a>&nbsp;<?php }?><a onClick="return window.confirm(&quot;单击“确定”继续。单击“取消”停止。&quot;);" href="?action=del&delid=<?php echo $a[id]?>" class="layui-btn layui-btn-danger layui-btn-mini ajax-delete" target="msgubotj">删除</a></td>
</tr>
<?php 
} 
