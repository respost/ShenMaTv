...
$type="id='$delid'";
dbdel(ubofxmx,$type);
echo msglayerurl("删除成功，返回页面",8,"distributor_list.php");
}
$array = $_POST["del_id"]; 
if(!empty($array)){
$del_sun=count($array); 
...
echo"<script>alert('删除成功');location.href='distributor_list.php'</script>";  
}
function hy($id)
{
...
<li class="layui-this"><a href="distributor_list.php">财务记录</a></li>
</ul>
...
<tr class="color1">
<th width="4%">&nbsp;</th>
<th width="18%" align="center" >推广人ID</th>
<th width="18%" align="center" >付费会员ID</th>
<th width="15%" align="center" >经验</th>
<th width="15%" align="center" valign="middle" >收入</th>
<th width="20%" align="center">记录时间</th>
...
if (is_numeric($name))
{
$user=getone("select * from ubouser WHERE user='".$name."'");
$uid=$user['id'];
}
$sql .=" and (uid='$uid' or user like '%$name%') ";
}
$result = mysql_query("select id from ubofxmx ".$sql."  ");
if($result == 0){
...
$query = mysql_query("select * from  ubofxmx  ".$sql." order by id desc   limit $offset, $Page_size");
