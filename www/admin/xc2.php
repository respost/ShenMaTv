...
$type="id='$delid'";
dbdel(ubozb,$type);
echo msglayerurl("删除成功，返回页面",8,"xc2.php");
}
$array = $_POST["del_id"]; 
if(!empty($array)){
$del_sun=count($array); 
...
upalldt(ubozb,$type);
echo msglayerurl("设置成功，返回页面",5,"xc2.php");
}
function fenlei($id)
{
...
<li class="layui-this"><a href="xc2.php">秀场管理</a></li>
</ul>
...
<input type="button" class="layui-btn" value="添加房间" onClick="location.href='xcadd.php'">
...
<th width="8%" align="center">房间上限</th>
<th width="8%" align="center">当前人数</th>
...
<td align="center"><?php $room=$a[room]; echo $room;?> 人</td>
<td align="center"><?php $enroll=$a[enroll]; echo $enroll;?> 人</td>
<td align="center"><?php echo $a[cishu]?> 次</td>
<td align="center"><?php echo $a[trends]?> 条</td>
<td align="center"><?php $switch=$a['switch']; $xcstate=$a[xcstate];if ($switch==1){echo "强制关闭";}elseif ($xcstate==1){echo "直播中";}else{echo "未开始";}?></td>
<td align="center">
<a href="zbdtgl.php?id=<?php echo $a[id]?>" class="layui-btn layui-btn-mini ajax-delete">动态</a>&nbsp;<a href="xcedit.php?id=<?php echo $a[id]?>" class="layui-btn layui-btn-normal layui-btn-mini">编辑</a>&nbsp;<a onClick="return window.confirm(&quot;单击“确定”继续。单击“取消”停止。&quot;);" href="?action=del&delid=<?php echo $a[id]?>" class="layui-btn layui-btn-danger layui-btn-mini ajax-delete"target="msgubotj">删除</a></td>
</tr>
<?php 
} 
...
?> 
<tr>
  <td colspan=21 style="padding-left:20px;" height="38"><label id="chkAll"><input class="demo--radioInput" style="margin:0;" type="checkbox" name="chkAll" id="chk" title="全选/反选" onClick="All(this, 'del_id[]')" />
