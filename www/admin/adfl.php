...
if($_POST[name]==null){
echo msglayer("名称不能为空！",8);
}else{
	$type="WHERE name='$_POST[name]'";
	$row=queryall(uboadfl,$type);  
	if($_POST[add]){
		$type="(`id`, `name`) VALUES (null, '$_POST[name]')";
		dbinsert(uboadfl,$type);
		echo msglayerurl("添加成功，返回页面",5,"adfl.php");
	}
	$id=$_GET[id];
	$type="WHERE id='$id'";
	$duqu=queryall(uboadfl,$type);
	if($_POST[edit]){
		$type="name='$_POST[name]'  where id='$id'";
		upalldt(uboadfl,$type);
		echo msglayerurl("修改成功，返回页面",4,"adfl.php");
	}
	$del=$_GET[action];
	$delid=$_GET[delid];
	if($del=="del"){
		$type="id='$delid'";
		dbdel(uboadfl,$type);
		echo msglayerurl("删除成功，返回页面",8,"adfl.php");
	}
}
...
<li class="layui-this"><a href="adfl.php">添加广告位</a></li>
</ul>
...
<div class="layui-tab layui-tab-brief">
<ul class="layui-tab-title">
<li class="layui-this"><a href="adfl.php">广告位管理</a></li>
</ul>
<div class="layui-tab-content">
<div class="layui-tab-item layui-show">
<table width="100%" class="layui-table" cellspacing="0" cellpadding="0">
...
<?php 
$Page_size=10; 
$result=mysql_query("SELECT * FROM uboadfl"); 
...
$query = mysql_query("select * from  uboadfl  limit $offset, $Page_size");
while ($a=mysql_fetch_array($query)) { 
?> 
<tr class="color3">
 <td height="30" align="center"><p align="center"><?php echo $a[id]?></p></td>
 <td align="center"><p align="center"><?php echo $a[name]?></p></td>
<td align="center"><p align="center"><a href="adfl.php?id=<?php echo $a[id]?>" class="layui-btn layui-btn-normal layui-btn-mini">编辑</a>&nbsp;<a  onclick="return checkdel()"  target="msgubotj"  href="?action=del&delid=<?php echo $a[id]?>" class="layui-btn layui-btn-danger layui-btn-mini ajax-delete" >删除</a> </p></td>
</tr>
<?php 
} 
