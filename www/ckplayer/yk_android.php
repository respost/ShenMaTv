<?php
/* 2014年8月6日 优酷视频解析 安卓版 默认返回mp4格式
...
* 调用方法：
* 1. 网址格式：yk_android.php?url=http://v.youku.com/v_show/id_XMTM4NTQ5MzM2NA==.html
* 1.1 网址格式：yk_android.php?url=http://player.youku.com/player.php/sid/XOTM1MTEwODQ0/v.swf
* 1.2 网址格式：yk_android.php?url=http://player.youku.com/player.php/Type/Folder/Fid/22981439/Ob/1/sid/XNzQ3Mjk2NzE2/v.swf
* 2. id格式：yk_android.php?id=XMTM4NTQ5MzM2NA==
* 仅供学习参考，勿用于商业用途
*/
error_reporting(0);
...
function getyk($vid){
    ...
    $stream_type = $stream[0]->stream_type;
    if ($stream_type == "flvhd") {
        Get_CW();
    }
    $segs = $stream[0]->segs;
    $key = $segs[0]->key;
    $ts = $segs[0]->total_milliseconds_video;
    $stream_fileid = $stream[0]->stream_fileid;
    $ep1 = urlencode(iconv("gbk", "UTF-8", base64_encode(yk_e("bf7e5f01", $sid . "_" . $stream_fileid . "_" . $token))));
    $kur = "http://k.youku.com/player/getFlvPath/sid/";
    $downlink = $kur . $sid . "_00/st/mp4/fileid/" . $stream_fileid . "?K=" . $key . "&hd=1&myp=0&ts=" . intval($ts / 1E3) . "&ypp=0&ymovie=1&ctype=12&ev=1&token=" . $token . "&oip=" . $oip . "&ep=" . $ep1;
    return $downlink;
}
