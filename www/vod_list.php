...
$type="concern=concern-1 where id=".$cuid;
if ($division==1){
upalldt(ubozb,$type);
}else{
upalldt(ubouser,$type);
}
$type="uid=".$cuid." and zbid=".$uid;
dbdel(se2sc,$type);
echo msglayerurl("已取消关注！",8,"vod_list.php?uid=".$uid);
exit;
}
$play="play.php";
if (empty($page))
...
function division($uid)
{
	$fname=getone("select * from ubozb WHERE division=1 and id=".$uid);
	if ($fname)
	{
    $return="1";
	}else{
...
<title><?php if ($uid>0) {echo $ncname."的视频";}else{echo $column;}?></title>
...
<span id="index"><i class="ico07"><img src="/img/logo.png" width="88px"<?php if($m || $pdid || $k || $sort){ ?> class="h"<?php } ?>/></i></span><i class="ico21"><?php if ($uid>0) {echo $ncname."的视频";}else{echo $column;}?></i>
...
<?php if ($uid>0){?>
<div class="huiyuan">
<div class="ui-avatar"><span style="background-image:url(<?php echo $avatar;?>)"></span></div>
<div class="login_t">
<h3><span style="color:#000">昵称：</span><?php echo $ncname;?> <a href="<?php if ($guanzhu2==1){;?>vod_list.php?uid=<?php echo $uid;?>&division=<?php echo $division;?>&style=1<?php }else{?>/plus/concern_favorite.php?id=<?php echo $uid;?>&division=<?php echo $division;?>&style=1<?php }?>" rel="external" target="msgubotj"><span id="gzzt_<?php echo $uid;?>"><?php echo $guanzhu1;?></span></a></h3>
<span class="login_lj" style="font-size:14px;">关注：<font color="red" size="3" id="concern_<?php echo $uid;?>"><?php echo $concern;?></font> 人 视频：<font color="red" size="3"><?php echo $trends;?></font> 个 财富：<font color="red" size="3" id="reward_<?php echo $uid;?>"><?php echo $money;?></font> 元</span></div>
</div>
<?php }else{?>
<div class="liebiao"><div class="left"><i></i>共有<span><?php echo $count;?></span>个</div><div class="right"><a href="vod_list.php?<?php if ($pdid){echo 'flid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=new<?php if ($s){echo '&s=vip';}?>" rel="external" class="ui-

link"><span <?php if ($sort=='new'){echo 'class="cur"';}?>>最新</span></a><a href="vod_list.php?<?php if ($pdid){echo 'flid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=heat<?php if ($s){echo '&s=vip';}?>" rel="external" class="ui-link"><span <?php if ($sort=='heat'){echo 

'class="cur"';}?>>人气</span></a><a href="vod_list.php?<?php if ($pdid){echo 'flid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=hot<?php if ($s){echo '&s=vip';}?>" rel="external" class="ui-link"><span <?php if ($sort=='hot'){echo 'class="cur"';}?>>点赞</span></a><a 

href="vod_list.php?<?php if ($pdid){echo 'flid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=fav<?php if ($s){echo '&s=vip';}?>" rel="external" class="ui-link"><span <?php if ($sort=='fav'){echo 'class="cur"';}?>>收藏</span></a></div></div>
<?php }?>
...
<script>$(function (){
	...
	//var ajaxurl = "xml/xml.php?flid=<?php echo $pdid;?>&sort=<?php echo $sort;?>&k=<?php echo $k;?>&m=<?php echo $m;?><?php if ($uid>0){echo "&uid=".$uid;}?><?php if ($s){echo '&s=vip';}?>";	
...
empty($s)){?>uboplay(\''+sqlJson[i].id+'\',\'ubosk\')<?php }else{?>pay()<?php }?>" >'+title+'</a></h4></li>';						
						}
...
<span onClick="uboplay('<?php echo $a[id]?>','ubosk')" style="background-image:url('<?php echo $a[pic]?>');background-size:224.64px auto;background-position:center center;"></span><?php }?>
